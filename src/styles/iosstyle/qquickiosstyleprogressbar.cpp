...
class QQuickIOSStyleProgressBarNode : public QQuickAnimatedNode
{
public:
    QQuickIOSStyleProgressBarNode(QQuickIOSStyleProgressBar *item);
...
QQuickIOSStyleProgressBarNode::QQuickIOSStyleProgressBarNode(QQuickIOSStyleProgressBar *item)
...
void QQuickIOSStyleProgressBarNode::updateCurrentTime(int time)
{
...
void QQuickIOSStyleProgressBarNode::sync(QQuickItem *item)
{
    QQuickIOSStyleProgressBar *bar = static_cast<QQuickIOSStyleProgressBar *>(item);
...
void QQuickIOSStyleProgressBarNode::moveNode(QSGTransformNode *transformNode, const QRectF &geometry, qreal progress)
{
...
QQuickIOSStyleProgressBar::QQuickIOSStyleProgressBar(QQuickItem *parent)
    : QQuickItem(parent)
{
    setFlag(ItemHasContents);
}

QColor QQuickIOSStyleProgressBar::color() const
{
    return m_color;
}

void QQuickIOSStyleProgressBar::setColor(const QColor &color)
{
    if (color == m_color)
        return;
...
qreal QQuickIOSStyleProgressBar::progress() const
{
    return m_progress;
}

void QQuickIOSStyleProgressBar::setProgress(qreal progress)
{
...
bool QQuickIOSStyleProgressBar::isIndeterminate() const
{
    return m_indeterminate;
}

void QQuickIOSStyleProgressBar::setIndeterminate(bool indeterminate)
{
...
void QQuickIOSStyleProgressBar::itemChange(QQuickItem::ItemChange change, const QQuickItem::ItemChangeData &data)
{
...
QSGNode *QQuickIOSStyleProgressBar::updatePaintNode(QSGNode *oldNode, QQuickItem::UpdatePaintNodeData *)
{
    QQuickIOSStyleProgressBarNode *node = static_cast<QQuickIOSStyleProgressBarNode *>(oldNode);
    if (isVisible() && width() > 0 && height() > 0) {
        if (!node)
            node = new QQuickIOSStyleProgressBarNode(this);
        node->sync(this);
    } else {
        delete node;
        node = nullptr;
